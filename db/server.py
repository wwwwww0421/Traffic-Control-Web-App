...
def handle_delete_session(iuser, imagic):
    ...
    ## alter as required
    response.append(build_response_refill('message', 'User Already Logged In.'))
    user = '!'
    magic = ''
...
def handle_login_request(iuser, imagic, parameters):
    ...
    #     response.append(build_response_refill('message', 'Please enter a valid username or password'))
    #     return [user, magic, response]
    username = parameters['usernameinput'][0]
    password = parameters['passwordinput'][0]
    ## alter as required (done-login one person)
    query = access_database_with_result("SELECT * FROM users WHERE username = '{}' AND password = '{}'".format(username, password))
    
    if len(query) > 0:
        response.append(build_response_redirect('/page.html'))
        magic = tokenise()
        now = int(time.time())
        logged_query = access_database_with_result("SELECT userid FROM users WHERE username = '{}'".format(username))
        userid = logged_query[0][0]
        access_database("INSERT INTO session (userid,magic,start,end) VALUES ('{}', {}, '{}', 0)".format(userid, magic, now))
        user = userid
...
def handle_logout_request(iuser, imagic, parameters):
    ...
    ## alter as required
    now = int(time.time())
    access_database("UPDATE session SET end='{}' WHERE magic='{}'".format(now, imagic))
    print('logout')
...
def handle_add_request(iuser, imagic, parameters):
    ...
    if handle_validate(iuser, imagic) != True:
        ...
        try:
            ...
            v_d = {"car": 0, "van":1, "truck":2,"taxi":3, "other":4, "motorbike":5, "bicycle":6, "bus":7}
            f_vehicle = v_d[vehicle]
            s_query = access_database_with_result("SELECT sessionid FROM session WHERE magic = '{}'".format(imagic))
            now = int(time.time())
            session_id = s_query[0][0]
            access_database("INSERT INTO traffic (sessionid, time, type, occupancy, location, mode) \
                             VALUES ('{}','{}','{}','{}','{}',1)".format(session_id, now, f_vehicle, occupancy, location))
            c_query = access_database_with_result("SELECT * FROM traffic WHERE mode=1 AND sessionid = {}".format(session_id))
            total = len(c_query)
            response.append(build_response_refill('message', 'Entry added.'))
            response.append(build_response_refill('total', str(total)))

        except KeyError:
            response.append(build_response_refill('message', 'Please enter a valid location'))
    return [user, magic, response]
...
def handle_undo_request(iuser, imagic, parameters):
    ...
    if handle_validate(iuser, imagic) != True:
        ...
        if 'locationinput' not in parameters.keys():
            response.append(build_response_refill('message', 'Please enter a valid location'))
        elif 'occpancyinput' in parameters.keys() or 'typeinput' in parameters.keys() or 'locationinput' in parameters.keys():   
            vehicle = parameters['typeinput'][0]
            v_d = {"car": 0, "van":1, "truck":2, "taxi":3, "other":4, "motorbike":5, "bicycle":6, "bus":7}
            ...
            try:
                query = access_database_with_result("SELECT recordid, sessionid FROM traffic \
                                                WHERE type = {} AND occupancy = {} AND location = '{}' AND mode = 1"
                                                .format(vehicle_f, occupancy, location))
                sessionid = query[0][1]
                recordid = query[-1][0]
                if len(query) > 0:
                    access_database("UPDATE traffic SET mode = 2 \
                                     WHERE recordid = {} AND sessionid = {} AND type = {} AND occupancy = {} AND location = '{}' AND mode=1"
                                     .format(recordid, sessionid, vehicle_f, occupancy, location))

                    #now = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime(float(time.time())))
                    now = int(time.time())
                    ## ALSO NEED TO CHANGE THE NEW ADDING ROW INTO mode=0
                    access_database("INSERT INTO traffic (sessionid, time, type, occupancy, location, mode) \
                                    VALUES ({}, {}, {}, {}, '{}', 0)"
                                    .format(sessionid, now, vehicle_f, occupancy, location))

                c_query = access_database_with_result("SELECT recordid FROM traffic WHERE mode=1")
                total = len(c_query)
                response.append(build_response_refill('message', 'Entry Un-done.'))
                response.append(build_response_refill('total', str(total)))
            except:
                response.append(build_response_refill('message', 'No record found. Please try again!'))
    user = ''
    magic = ''
    return [user, magic, response]
...
def handle_back_request(iuser, imagic, parameters):
    ...
    user = iuser
    magic = imagic
    query = ("SELECT * FROM session WHERE userid = {} AND magic = {} AND end != 0".format(iuser, imagic))
    ## alter as required
    if handle_validate(iuser, imagic) != True:
        print('handle_validate bakc')
        response.append(build_response_redirect('/index.html'))
    else:
...
def handle_summary_request(iuser, imagic, parameters):
    ...
    if handle_validate(iuser, imagic) != True:
        #Invalid sessions redirect to login
        print('handle_validate summary')
        response.append(build_response_redirect('/index.html'))
    else:
        #query = access_database_with_result("SELECT type, COUNT(type) FROM traffic WHERE mode = 1 OR mode = 2 GROUP BY type")
        sum_car = access_database_with_result("SELECT COUNT(type) FROM traffic WHERE (type = 0 AND mode = 1)")
        sum_taxi = access_database_with_result("SELECT COUNT(type) FROM traffic WHERE (type = 3 AND mode = 1)")
        sum_bus = access_database_with_result("SELECT COUNT(type) FROM traffic WHERE (type = 7 AND mode = 1)")
        sum_motorbike = access_database_with_result("SELECT COUNT(type) FROM traffic WHERE (type = 5 AND mode = 1)")
        sum_bicycle = access_database_with_result("SELECT COUNT(type) FROM traffic WHERE (type = 6 AND mode = 1)")
        sum_van = access_database_with_result("SELECT COUNT(type) FROM traffic WHERE (type = 1 AND mode = 1)")
        sum_truck = access_database_with_result("SELECT COUNT(type) FROM traffic WHERE (type = 2 AND mode = 1)")
        sum_other = access_database_with_result("SELECT COUNT(type) FROM traffic WHERE (type = 4 AND mode = 1)")

        total = sum_car[0][0] + sum_taxi[0][0] + sum_bus[0][0] + sum_bicycle[0][0] + sum_motorbike[0][0] + sum_van[0][0] + sum_truck[0][0] + sum_other[0][0]
        ...
        response.append(build_response_refill('total', total))
        user = ''
        magic = ''
    return [user, magic, response]
...
### OUTPUT CSV FILES
            ...
            start_time_today = time.mktime(datetime.strptime(f'{today} 00:00:00', "%Y-%m-%d %H:%M:%S").timetuple())
            end_time_today = time.mktime(datetime.strptime(f'{today} 23:59:59', "%Y-%m-%d %H:%M:%S").timetuple())
            
            week_today = dt.date.today()
            last_week = (week_today - dt.timedelta(days=6))
            start_time_week = time.mktime(datetime.strptime(f'{last_week} 00:00:00', "%Y-%m-%d %H:%M:%S").timetuple())
            end_time_week = time.mktime(datetime.strptime(f'{week_today} 23:59:59', "%Y-%m-%d %H:%M:%S").timetuple())
            ...
            finding_month_start = "{}-{:02d}-{:02d} 00:00:00".format(year, last_month, last_date)
            finding_month_end = "{}-{:02d}-{:02d} 23:59:59".format(year, month, day)
            start_time_month = int(dt.datetime.strptime(f'{finding_month_start}', '%Y-%m-%d %H:%M:%S').strftime("%s"))
            end_time_month = int(dt.datetime.strptime(f'{finding_month_end}', '%Y-%m-%d %H:%M:%S').strftime("%s"))

            day_query = access_database_with_result("SELECT username, start, end FROM session \
                                                     INNER JOIN users ON session.userid = users.userid \
                                                     WHERE (start BETWEEN {} AND {}) AND (end BETWEEN {} AND {}) AND end != 0"
                                                     .format(start_time_today, end_time_today, start_time_today, end_time_today))
            special_day_query = access_database_with_result("SELECT username, start, end FROM session \
                                                             INNER JOIN users ON session.userid = users.userid \
                                                             WHERE (start NOT BETWEEN '{}' AND '{}') AND (end BETWEEN '{}' AND '{}') AND end != 0"
                                                             .format(start_time_today, end_time_today, start_time_today, end_time_today))
            
            week_query = access_database_with_result("SELECT username, start, end FROM session \
                                                      INNER JOIN users ON session.userid = users.userid \
                                                      WHERE (start BETWEEN {} AND {}) AND (end BETWEEN {} AND {}) AND end != 0"
                                                      .format(start_time_week, end_time_week, start_time_week, end_time_week))
            ## starting date not in what we want
            special_week_query = access_database_with_result("SELECT username, start, end FROM session \
                                                                INNER JOIN users ON session.userid = users.userid \
                                                                WHERE (start NOT BETWEEN {} AND {}) AND (end BETWEEN {} AND {}) AND end != 0"
                                                                .format(start_time_week, end_time_week, start_time_week, end_time_week))
            
            month_query = access_database_with_result("SELECT username, start, end FROM session \
                                                       INNER JOIN users ON session.userid = users.userid \
                                                       WHERE (start BETWEEN {} AND {}) AND (end BETWEEN {} AND {}) AND end != 0"
                                                       .format(start_time_month, end_time_month, start_time_month, end_time_month))
            special_month_query = access_database_with_result("SELECT username, start, end FROM session \
                                                               INNER JOIN users ON session.userid = users.userid \
                                                               WHERE (start NOT BETWEEN {} AND {}) AND (end BETWEEN {} AND {}) AND end != 0"
                                                               .format(start_time_month, end_time_month, start_time_month, end_time_month))
            ...
            if len(special_month_query) > 0:
                for i in special_month_query:
                    diff_month = [[i[0], i[2] - start_time_month] for i in special_month_query]
                    if i[0] == 'test1':
                        test1[2] += i[1]
                    elif i[0] == 'test2':
                        test2[2] += i[1]
            ...
            text += "test1,{},{},{}\n".format(round((test1[0]/3600),1), round((test1[1]/3600),1), round((test1[2]/3600),1))
            text += "test2,{},{},{}\n".format(round((test2[0]/3600),1), round((test2[1]/3600),1), round((test2[2]/3600),1))
            text += "test3,{},{},{}\n".format(round((test3[0]/3600),1), round((test3[1]/3600),1), round((test3[2]/3600),1))
            text += "test4,{},{},{}\n".format(round((test4[0]/3600),1), round((test4[1]/3600),1), round((test4[2]/3600),1))
            text += "test5,{},{},{}\n".format(round((test5[0]/3600),1), round((test5[1]/3600),1), round((test5[2]/3600),1))
            text += "test6,{},{},{}\n".format(round((test6[0]/3600),1), round((test6[1]/3600),1), round((test6[2]/3600),1))
            text += "test7,{},{},{}\n".format(round((test7[0]/3600),1), round((test7[1]/3600),1), round((test7[2]/3600),1))
            text += "test8,{},{},{}\n".format(round((test8[0]/3600),1), round((test8[1]/3600),1), round((test8[2]/3600),1))
            text += "test9,{},{},{}\n".format(round((test9[0]/3600),1), round((test9[1]/3600),1), round((test9[2]/3600),1))
            text += "test10,{},{},{}\n".format(round((test10[0]/3600),1), round((test10[1]/3600),1), round((test10[2]/3600),1))
            ...
            today = time.strftime("%Y-%m-%d", time.gmtime(float(time.time())))
            start_time = time.mktime(datetime.strptime(f'{today} 00:00:00', "%Y-%m-%d %H:%M:%S").timetuple())
            end_time = time.mktime(datetime.strptime(f'{today} 23:59:59', "%Y-%m-%d %H:%M:%S").timetuple())
            query = access_database_with_result("SELECT location, type, occupancy,COUNT(*) FROM traffic \
                                                 WHERE mode=1 AND time BETWEEN {} AND {} \
                                                 GROUP BY location, type, occupancy"
            ...
            for i in range(len(location)):
                ...
                text += '"{}", {}, {}, {}, {}, {}\n'.format(location[i], type[i], occupancy[i][0], occupancy[i][1], occupancy[i][2], occupancy[i][3])
